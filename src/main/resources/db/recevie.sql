...
select job.*,
       ...
       (
           CASE
               WHEN score.score = null THEN
                   '0'
               ELSE
                   '1'
               END
           ) AS STATUS
from (SELECT DATE_FORMAT(b.createTime, '%Y-%m-%d %h:%m:%s') as time,
             b.*,
             c.coursename
      FROM job b,
           teachclass c
      WHERE b.teachclassid in
            (SELECT a.classID
             FROM studentclass a
             WHERE a.studentno = '162210201321')
        AND c.ID = b.teachclassid
        AND c.ID = 2
      ORDER BY b.ID DESC) as job
         LEFT JOIN (SELECT jobID, score from score where studentno = '162210201321') as score
